                ...
                <div class="flex-fill p-0   d-flex flex-column justify-content-center rounded-pill-custom-1">
                    ...
                    <button style="background: transparent;border:0;" @click="goback">
                        <div class="card goback flex-row flex-md-column m-0 card-height justify-content-center align-items-center">
                            <i :class="menu_administrador_reportes.goback.icon"></i>
                            <div class="title text-uppercase mt-1">
                            {{menu_administrador_reportes.goback.title}}
                            </div>
                        </div>
                    </button>
                </div>
            </div>
            <div :class="['order-1 order-md-2 d-flex flex-wrap pt-1 flex-md-fill overflow-auto',{'align-content-md-center':menu_administrador_reportes.menu.length <= 6},{'align-content-md-start':menu_administrador_reportes.menu.length > 6}]">
                <div v-for="(item,index) in menu_administrador_reportes.menu" :key="index" class="col-12 col-md-6 col-lg-4 col-xl-4 col-xxl-4  ">
                    <a :href="item.href">
...
export default {
    ...
    methods: {
        goback(){
            this.$router.push("/auth/menu_inicial_administrador");
        }
    },
    mounted() {
    },
