...
const store = new Vuex.Store({
    ...
    mutations: {
        ...
        updateItemFormField(state, form) {
            return state[ 'formReservacionQuirofano' ][ 'intervencion' ][form.index][form.fieldName] = form.fieldValue;
        },
        SET_FILE(state, file) {
        ...
        editFormReservacionQuirofano(state, form) {
            let temp = JSON.parse(originalFormReservacionQuirofano);
                ...
                temp['area_intervencion'] = form.area_intervencion;
                ...
                temp['h_inicio'] = (form.h_inicio.split(" "))[1]
            ...
            let idBuscado = form.area_intervencion ==="" ? "" :  Number(form.area_intervencion); // Puedes cambiar este valor al ID que desees buscar
                console.log("id_buscado",idBuscado);

                if (!is_empty(idBuscado)) {
                    document.getElementById('level4').value = idBuscado
        ...
        deleteSolicitudQx( state,data){
           // console.log(index)
            return state.listadoSolicitudesQx[ data[0] ]['dias'] = state.listadoSolicitudesQx[ data[0] ]['dias'].filter((x,index)=>Number(index) !== Number(data[1]) );
        },
