<template>
    <div ref="cintillo_superior" :class="['container-fluid px-0 mb-1 ', getMaximize]">
        <div class="col-12 col-md-4 mx-auto">
            <date-picker style="text-align: center !important;" v-model="rango"
                placeholder="Pulse aquí para buscar por rango de fechas" range :language="'spanish'"
                @change="fetchData">
            </date-picker>
                            ...
                            <div class="ml-2 d-flex justify-content-center">
                                <div class="d-flex px-2 flex-column align-items-center">
                                    ...
                                    <div style="line-height: 1;">
                                        {{ $store.state.listadoSolicitudesQx.map(x => x['dias']).flat().filter(x =>
        x.h_fin
        !== null).length }}</div>
                                </div>
                                ...
                                <div class="d-flex px-2 flex-column align-items-center">
                                    ...
                                    <div style="line-height: 1;">
                                        {{ $store.state.listadoSolicitudesQx.map(x => x['dias']).flat().length }}</div>
                                </div>
                            </div>
                        </b>
                    </div>
                            ...
                            <div class="ml-2 d-flex justify-content-center">
                                <div class="d-flex px-2 flex-column align-items-center">
                                    ...
                                    <div style="line-height: 1;">
                                        {{ $store.state.listadoSolicitudesQx.map(x => x['dias']).flat().filter(x =>
        x.h_fin
        !== null && [418, 420, 421].includes(Number(x['area_intervencion']))
    ).length }}</div>
                                </div>
                                <div class="mx-1 text-center"><b class=" border-left border-white"></b></div>
                                <div class="d-flex px-2 flex-column align-items-center">
                                    ...
                                    <div style="line-height: 1;">
                                        {{ $store.state.listadoSolicitudesQx.map(x => x['dias']).flat().filter(x =>
        [418, 420, 421].includes(Number(x['area_intervencion']))).length }}</div>
                                </div>
                            </div>
                        </b>
                    </div>
                            ...
                            <div class="ml-2 d-flex justify-content-center">
                                <div class="d-flex px-2 flex-column align-items-center">
                                    ...
                                    <div style="line-height: 1;">
                                        {{ $store.state.listadoSolicitudesQx.map(x => x['dias']).flat().filter(x =>
        x.h_fin
        !== null && [419, 422, 423, 424, 425].includes(Number( x['area_intervencion'] )
                                        )
                                        ).length}}</div>
                                </div>
                                <div class="mx-1 text-center"><b class=" border-left border-white"></b></div>
                                <div class="d-flex px-2 flex-column align-items-center">
                                    ...
                                    <div style="line-height: 1;">
                                        {{$store.state.listadoSolicitudesQx.map(x=>x['dias']).flat().filter(x=>
                                        [419,422,423,424,425].includes( Number( x['area_intervencion'] ) ) ).length}}
                                    </div>
                                </div>
                            </div>
                        </b>
...
<script>
    import { fechaAMD2, get } from "../../../../../helpers/customHelpers.js";
            ...
            async getSolicitudesQx() {
                try {
                    ...
                    if (this.$route.path == "/areaQuirofano/index_quirofano") {
                        model = await get(location.origin + `/areaQuirofano/cupo/getAllInterno?startDate=${fechaAMD2(startDate)}&endDate=${fechaAMD2(endDate)}&area_intervencion=hospitalizacion`)
                    } else {
