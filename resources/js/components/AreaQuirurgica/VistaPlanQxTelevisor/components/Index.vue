                                ...
                                <tbody>
                                    <tr v-for="(solicitud, index) in item" :key="index"
                                        ...
                                        <td class="tbl-row-radius-left py-0 text-center text-white p-1">
                                            ...
                                            <h3 v-if="[1,2,3,4,5,6,7,8,9].includes(solicitud['n_quirofano'])"
                                                class="mb-0" :style="{ 'color': 'white' }">
                                                QX{{ solicitud['n_quirofano'] }}
                                            </h3>
                                            <h3 v-else-if="[10].includes(solicitud['n_quirofano'])" class="mb-0"
                                                :style="{ 'color': 'white' }">
                                                SEE
                                            </h3>
                                            <h3 v-else class="mb-0" :style="{ 'color': 'red' }">
                                                P
                                            </h3>
                                            ...
                                            <div :class="{ 'flex-column h5 mb-0 text-clave-espera d-none': true, 'd-flex': solicitud.fase_ubicacion === 'En espera de clave' }"
                                                ...
                                                <div class="text-nowrap"><i
                                                        :class="{ 'fas fa-key': true, 'd-none': false }"></i> En espera
                                                    de
                                                    clave</div>
                                        ...
                                        <td class="text-center py-0  text-white">
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div :class="[(solicitud['genero'] == 'f' ? 'badge-pink' : 'badge-primary'), 'badge mb-1 d-flex align-items-center px-2 mr-2 text-uppercase']"
                                                    style="font-size: 0.6rem; padding: 0.1rem;">
                                                    <i class="fas fa-venus-mars mr-2"></i>
                                                    <span style="font-size: 0.9rem; font-weight: 500;">{{
                                                        solicitud['genero'] }}</span>
                                                </div>
                                        ...
                                        <td class="text-center py-0  text-white">
                                            <div :class="[
                                                    ...
                                                    { 'd-none': solicitud.fase_ubicacion === 'En espera de clave' || getHistorialHorasQx(solicitud).length > 1 || !solicitud['h_inicio'] && !solicitud['h_real_inicio']}, 
                                                    { 'd-flex': solicitud.fase_ubicacion !== 'En espera de clave' && getHistorialHorasQx(solicitud).length === 1 }
                                                ]" title="Hora pautada de inicio">
                                                <i class="fas fa-clock mr-1"></i>
                                                {{ solicitud['h_inicio'] ? get_hora_inicio(solicitud['h_inicio']) :
                                                '11111--:--' }}
                                            </div>
                                            <div :class="[
                                                    { 'd-none': getHistorialHorasQx(solicitud).length === 1 || solicitud.fase_ubicacion === 'En espera de clave' }, 
                                                    'text-center text-nowrap align-items-center'
                                                ]" title="Hora real de inicio">
                                                <i v-if="solicitud['h_real_inicio']" class="pc-clock-update mr-1"></i>
                                                {{ solicitud['h_real_inicio'] ? horaAMPM(
                                        ...
                                        <td class="text-center py-0  text-white">
                                            ...
                                            <div
                                                ...
                                                <div v-if="solicitud['h_fin'] !== null && solicitud['fase_ubicacion'] !== 'En quirófano'"
                                                    class="d-flex flex-column mb-1 justify-content-start p-1"
                                        ...
                                        <td class="text-white py-0 ">
                                            ...
                                            <ul v-if="JSON.parse(solicitud.intervencion).length > 0"
                                                ...
                                                <li class="d-flex flex-column" :key="index3" style="font-size:0.9rem"
                                                    ...
                                                    <div v-if="is_object(item3.description) && item3.hasOwnProperty('description')"
                                                        ...
                                                        <div v-if="item3.hasOwnProperty('description')"
                                                            class="d-flex mb-1">
                                                            <div v-if="item3.description.codigo !== ''"
                                                                class="badge badge-warning mr-1">
                                                                {{ item3.description.codigo }}</div>
                                                            <div v-if="item3.description.kitsum_asociado !== ''"
                                                    ...
                                                    <div
                                                        v-if="!is_object(item3.description) && item3.hasOwnProperty('description')">
                                                        {{ item3.description }}
                                                    </div>
                                        ...
                                        <td class=" text-white py-0 ">
                                            ...
                                            <ul class="list-group mt-1 list-group-flush">
                                                <li v-for="(item4, index4) in getSetCirujanos(JSON.parse(solicitud.intervencion))"
                                                    :key="index4" class="list-group-item border-0 bg-transparent p-0"
                                                    style="font-size:0.9rem">
                                                    <div class="d-flex align-items-center">
                                                        <img loading="lazy"
                                                            onerror="this.src='/image/system/patient.svg'"
                                                            :src="getImagen(item4)" class="img-doctor mr-1"
                                        ...
                                        <td class=" text-white py-0 ">
                                            <div v-if="JSON.parse(solicitud.intervencion).length > 0">
                                                <ul v-for="(item3, index3) in JSON.parse(solicitud.intervencion)"
                                                    :key="index3" class="list-group list-group-flush">
                                                    <li v-for="(item4, index4) in   item3['anestesiologo']"
                                                        :key="index4"
                                        ...
                                        <td class=" text-white py-0 ">
                                            <ul v-for="(item3, index3) in JSON.parse(solicitud.intervencion)"
                                                :key="index3" class="list-group list-group-flush">
                                                <li v-for="(item4, index4) in   item3['equipos_especiales']"
                                                    :key="index4" class="list-group-item bg-transparent p-0"
                                                    style="font-size:0.9rem">
                                                    {{ item4.description }}
                                                </li>
                                        ...
                                        <td :class="{
                                            ...
                                            <div v-if="Number(solicitud.area_intervencion) === 418"
                                                class="d-flex align-items-center">
                                                <div style="width:10px;height:10px;"
                                                    class="bg-warning rounded-circle mr-1">
                                                </div>
                                                <div>Hosp.</div>
                                            </div>
                                            <div v-if="Number(solicitud.area_intervencion) === 422"
                                                class="d-flex align-items-center">
                                                <div style="width:10px;height:10px;"
                                                    class="bg-info rounded-circle mr-1">
                                                </div>
                                                <div>Ambu.</div>
                                            </div>
