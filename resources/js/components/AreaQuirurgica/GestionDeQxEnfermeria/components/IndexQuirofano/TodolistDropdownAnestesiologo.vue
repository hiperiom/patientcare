<template>
    <div >
        ...
        <div v-for="(item, index) in ObjectData" :key="index" class="d-flex rounded align-items-center shadow-sm mb-1">
            <div class="flex-shrink-1  btn-group">
                ...
                <div @click="dropdownStop" class="dropdown-menu dropdown-menu-left p-1" aria-labelledby="triggerId">
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <input @keypress="filterValues(index)" :ref="'searchInput'+index+user_id_paciente" type="text" class="form-control mb-1">

                        <div class="list-group overflow-auto" :ref="'myList'+index+user_id_paciente" style="max-height:40vh">
                            <a href="#" v-for="(personal, index_personal) in $store.state.personal_salud" :key="index_personal" :data-index="index"  :data-user_id="personal.user_id" :data-description="personal.medico" :class="['list-group-item p-1 list-group-item-action',{'active':getPersonalComputed.map(x=>x.user_id).includes(Number(personal.user_id))}]"> 
                                {{personal.medico}}
                            </a>
            ...
            <div class="flex-grow-1 pr-1">
                <ul class="list-group  list-group-flush p-0">
                   <li v-for="(item2,index2) in item" :key="index2" class="list-group-item  text-nowrap d-flex justify-content-between align-items-center p-0" >
                       ...
                       <button @click="handleDestroyPersonal(item2.id,index)" :data-id="item2.id" :data-index="index2"   class="btn btn-default btn-sm ml-1 border-0 text-secondary">&#128939;</button>
                    </li>
...
<script>
    ...
    export default {
        ...
        methods: {
            filterValues(index){
                let searchInput = this.$refs["searchInput"+index+this.user_id_paciente][0];
                    let searchText = searchInput.value.toLowerCase();
                    let listItems = this.$refs['myList'+index+this.user_id_paciente] //.querySelectorAll("a");
        ...
        mounted(){
            this.ObjectData.forEach( (item,index)=>{
                ...
                this.$refs["myList"+index+this.user_id_paciente][0].addEventListener("click",(e)=>{
                    this.user_id_medico = e.target.dataset.user_id
                    this.description = e.target.dataset.description
                    this.index_anestesiologo = e.target.dataset.index
