<template>
    <div class="dropdown w-100 m-0">
        <button class="btn-link btn btn-block font-weight-bold border-0 dropdown-toggle d-flex align-items-center justify-content-center overflow-hidden" :style="{'color':textColor}" type="button" id="triggerId" data-toggle="dropdown" aria-haspopup="true"
                aria-expanded="false">
                <i class="pc-medico"></i> {{ title }}
        </button>
        <div @click="dropdownStop" class="dropdown-menu p-3" aria-labelledby="triggerId">
            <h6 :style="{'color':textColor}" class="dropdown-header"><i class="pc-medico"></i> {{ title }}</h6>
            <div class="d-flex justify-content-between">
                <select
                    ref="recuperacion"
                    class="form-control bg-gray-footer-parodi mr-1"
                    v-model="formData['recuperacion']"
                >
                    <option value="">Seleccione</option>
                    <option
                        v-for="(personal, index_personal) in getPersonalComputed"
                        :key="index_personal"
                ...
                <button
                    @click="handleAddPersonal('recuperacion')"
                    class="btn btn-info"
                >+</button>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            getPersonal(){
                return this.$store.state.personal_salud.filter(x=>{
                    if(
                        ['Instrumentista','Circulante Anestesia','Circulante Cirugía'].includes( x['equipo_salud']) &&
                        !(this.$store.state.otroPersonalAsistencial.some( z=>Number(z['user_id']) === Number(x['user_id']) && Number(z['active']) ===1))
                    ){
                        return x
                    }
                })
