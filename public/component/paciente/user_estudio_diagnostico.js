    ...
    let response = await store()
        if ( !is_null(response) ) {
            let index_id = pacientes_datos.findIndex( x => equalsInt( x.user_id,response.user_id ) )
                ...
                pacientes_datos[ index_id ]['estudios'].push( response )
...
let handleUpdate = async ()=>{
    ...
    let response = await update(estudio_id)
        if ( !is_null(response) ) {
            ...
            let index_estudio_id = pacientes_datos[ index_user_id ]['estudios'].findIndex(x=> equalsInt( x.id , estudio_id ) )

                Object.assign(pacientes_datos[ index_user_id ]['estudios'][ index_estudio_id ], response)
                ...
                Toast.fire({
                    icon: message['image'],
                    title: message['title'],
                    text: "Estudio actualizado correctamente",
                    didClose: () => {
                        setTimeout(() => {
                            reset_reset()
                            index()
...
let getAll = ()=>{
        ...
        let index_id = pacientes_datos.findIndex( x => equalsInt( x.user_id, user_id ) )
            return pacientes_datos[ index_id ]['estudios']
    }
let show = async ()=>{
        //pacientes_datos
    }
let store = async ()=>{
        let formData = new FormData();
            ...
            return await post("/consultaexterna/user/paciente/estudio/store",formData)
    }
let handleDestroy = async (estudio_id)=>{
    Toast.fire({
        icon:"warning",
        title: "Atención",
        text: "¿Desea eliminar este Estudio?",
        showDenyButton: true,
        ...
        didClose: async () => {
            ...
            if (response) {
                let user_id = useState.user_id
                let index_id = pacientes_datos.findIndex( x => equalsInt( x.user_id, user_id ) )
                pacientes_datos[ index_id]['estudios'] = pacientes_datos[ index_id]['estudios'].filter( x=>{
...
/* let print_color = (user_id,episodio_id)=>{

    window.open(`/pdf/estudio/preview/${user_centro_salud_id}/${episodio_id}/${user_id_medico}/${user_id}/color`)
    } */
let print_pdf = (user_id,episodio_id,print)=>{
    window.open(`/consultaexterna/pdf/estudio/preview/${user_centro_salud_id}/${episodio_id}/${user_id_medico}/${user_id}/${print}`)
    }
let compartir_ws = (user_id,print)=>{
    let index_id = pacientes_datos.findIndex( x => equalsInt( x.user_id, user_id ) )
    let paciente = pacientes_datos[ index_id ]
    let episodio_id = paciente.episodio
    window.open(`https://wa.me/${paciente.telefono_movil}?text=Hola! ${medico_genero=="f"?"La ":"El "}${medico_fullname}, te ha enviado una solicitud de estudio diagnóstico. Pulsa en el siguiente enlace para descargarlo: ${host_patientcare}/pdf/estudio/ws/${user_centro_salud_id}/${episodio_id}/${user_id_medico}/${user_id}/${print}`)
    }
let compartir_email = async (user_id,print)=>{
    ...
    Toast.fire({
        icon:"warning",
        title: "Atención",
        text: `¿Desea enviar la Solicitud de Estudio Diagnóstico al correo ${paciente.email}?`,
        showDenyButton: true,
        showCancelButton: false,
        confirmButtonText: `Si`,
        didClose: async () => {
            let response = await get(`/consultaexterna/pdf/estudio/email/${user_centro_salud_id}/${episodio_id}/${user_id_medico}/${user_id}/${print}`)
            ...
            if (response) {
                ...
                Swal.fire(
                    'Enviado!',
                    `La Solicitud de Estudio Diagnóstico se ha sido enviado a ${paciente.email} exitosamente`,
                    'success'
                    )
            }else{
                Swal.fire(
...
let handleEdit = (estudio_id)=>{
    ...
    let user_id = useState.user_id
    let index_id = pacientes_datos.findIndex( x => equalsInt( x.user_id, user_id ) )
    let model = first( pacientes_datos[ index_id]['estudios'].filter( x=> parseInt(x.id) === parseInt(estudio_id) ) )
...
export let renderHtml = async (user_id)=>{
    let $modal = d.querySelector(`#modelId `)
        ...
        let $html = entById('artefacto_0012').content
            $html = d.importNode($html,true)
            ...
            d.querySelector('#modelId .modal-body #imprimir_estudio_color').dataset.user_id=user_id
            d.querySelector('#modelId .modal-body #imprimir_estudio_bw').dataset.user_id=user_id

            d.querySelector('#modelId .modal-body #whatsapp_estudio_color').dataset.user_id=user_id
            d.querySelector('#modelId .modal-body #whatsapp_estudio_bw').dataset.user_id=user_id

            d.querySelector('#modelId .modal-body #email_estudio_color').dataset.user_id=user_id
            d.querySelector('#modelId .modal-body #email_estudio_bw').dataset.user_id=user_id
