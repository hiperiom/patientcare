class UserCuestRecipe {
    static create(selector, user_id) {
        ...
        $("#user_cuest_model_store").on("click", function() {
            if (UserCuestRecipe.validate()) {
                UserCuestRecipe.store(user_id)
                    .then(response => {
    ...
    static store(user_id) {
        ...
        return post( location.origin+"/user_cuest_recipe/store", formdata)
    }
    ...
    static destroy(id, user_id) {
        ...
        if (r == true) {
            get( location.origin+"/user_cuest_recipe/destroy/" + id)
                .then(response => {
                    UserCuestHistoria.index('.modal-body', user_id)
                })
        }
