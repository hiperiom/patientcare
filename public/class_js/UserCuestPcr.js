class UserCuestPcr {
    static index(selector, user_id) {
        ...
        post( location.origin+"/user_cuest_pcr/index", formdata)
            .then(response => {
                let filas = "";
                let tempRow = "";
                let dia = "";
                    ...
                    $(response).each(function(index, element) {
                        ...
                        tempRow = `
                            <tr id="model_${element.id}">
                                ...
                                <td class="text-center font-weight-bold ${UserCuestPcr.colorPcr(element.value)}" id="modal-model-${element.id}">${element.value}</td>
                                ...
                                <td align="center">
                                <!-- <button title="Eliminar valor" onclick="UserCuestPcr.eliminar(${element.id},${user_id})" class="delete-row btn btn-danger" data-option="${element.id}"><i class="fa fa-minus" aria-hidden="true"></i></button>-->
                                </td>
                            </tr>
                        `;
                        filas = filas + tempRow
        ...
        $("#user_cuest_model_create").on('click', function() {
            UserCuestPcr.create(".modal-body", user_id)
        });
    ...
    static create(selector, user_id) {
        ...
        $("#regresar").on("click", function() {
            UserCuestPcr.index(selector, user_id)
        });
        $("#user_cuest_model_store").on("click", function() {
            UserCuestPcr.store(user_id)
                .then(response => {
                    let valor = $("#value").val();
                    ...
                    UserCuestPcr.index('.modal-body', user_id)
                });
        });
    ...
    static store(user_id) {
        ...
        formdata.append("_token", $("#_token").val())
        return post( location.origin+"/user_cuest_pcr/store", formdata)
    }
    static destroy(id) {
        let formdata = new FormData();
    ...
    static columnaValor(selector, data) {
        $(selector).html(`
            <div onclick="UserCuestPcr.index('.modal-body',${data.user_id})" class="text-center text-secondary font-weight-bold cursor-pointer">
                <div class="text-center text-gray w-100 title-columna">
                    PCR
                </div>
                ${data.valor=="" || data.valor==null?"<a class='btn btn-light btn-block' data-tippy-content='Pulse para añadir nuevo valor'>Añadir</a>":"<span class='"+UserCuestPcr.colorPcr(data.valor)+"'>"+data.valor+"</span>"}
            </div>
        `);
    }
}
