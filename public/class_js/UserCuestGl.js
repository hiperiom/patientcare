...
export default class UserCuestGl {
    static index(selector, user_id,episodio_id) {
        ...
        post( location.origin+"/user_cuest_gl/show", formdata)
            .then(response => {
                let filas = "";
                let tempRow = "";
                let dia = "";
                    ...
                    $(response).each(function(index, element) {
                        ...
                        tempRow = `
                            <tr id="model_${element.id}">
                                ...
                                <td class="text-center font-weight-bold ${UserCuestGl.colorGl(element.value,'historial')}" id="modal-model-${element.id}">${element.value} mg/dL</td>
                                ...
                                <td align="center">
                                <!-- <button title="Eliminar valor" onclick="UserCuestGl.eliminar(${element.id},${user_id})" class="delete-row btn btn-danger" data-option="${element.id}"><i class="fa fa-minus" aria-hidden="true"></i></button> -->
                                </td>
                            </tr>
                        `;
                        filas = filas + tempRow
        ...
        $("#user_cuest_model_create").on('click', function() {
            UserCuestGl.create(".modal-body", user_id,episodio_id)
        });
    ...
    static create(selector, user_id,episodio_id) {
        ...
        $("#regresar").on("click", function() {
            UserCuestGl.index(selector, user_id,episodio_id)
        });
        $("#user_cuest_model_store").on("click", function() {
            UserCuestGl.store(user_id,episodio_id)
                .then(response => {
                    let valor = $("#value").val();
                    ...
                    UserCuestGl.index('.modal-body', user_id,episodio_id)
                });
        });
       /*  $("#info").on("click", function() {
            UserCuestGl.help(selector, user_id);
    ...
    static store(user_id,episodio_id) {
        ...
        formdata.append("_token", $("#_token").val())
        return post( location.origin+"/user_cuest_gl/store", formdata)
    }
    ...
    static columnaValor(selector, data) {
        $(selector).html(`
            <div onclick="UserCuestGl.index('.modal-body',${data.user_id},${data.episodio_id})" class="text-center text-secondary font-weight-bold cursor-pointer">
                <div class="text-center text-gray w-100 title-columna">
                    GL
                </div>
                ${data.valor=="" || data.valor==null?"<a class='btn btn-light btn-block' data-tippy-content='Pulse para añadir nuevo valor'>Añadir</a>":"<span class='"+UserCuestGl.colorGl(data.valor)+"'>"+data.valor+" mg/dL</span>"}
            </div>
        `);
    }
}
