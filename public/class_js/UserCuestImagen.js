class UserCuestImagen {
        ...
        $("#file").on("change", function() {
            ...
            $("#file_temp").html(`
                <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 col-xl-2 text-center">
                    <img class="img-fluid" style="height: 4em;width: 4em;margin: 0.5em;" src="/image/system/icono_estudio.svg">
                </div>
            `);
        });
        $("#user_cuest_model_store").on("click", function() {
            if (UserCuestImagen.validate()) {
                UserCuestImagen.store(user_id)
                    .then(response => {
    ...
    static store(user_id) {
        ...
        return post(location.origin+"/user_cuest_imagen/store", formdata)
    ...
    static destroy(id, user_id) {
        ...
        if (r == true) {
            get(location.origin+"/user_cuest_imagen/destroy/" + id)
                .then(response => {
                    UserCuestHistoria.index('.modal-body', user_id)
                })
        }
