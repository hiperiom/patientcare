...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    ...
    ObjectData: function ObjectData() {//return JSON.parse(this.$store.state.formReservacionQuirofano[this.name])
    }
  },
  methods: {
    getActiveColor: function getActiveColor(item) {
      return ''; //this.getPersonal().map(x => x['user_id']).includes(item.user_id) ? 'active-' + this.color : ''
    },
    filterValues: function filterValues() {
    ...
    getPersonal: function getPersonal(tipo_personal) {
      ...
      /* } */
      //console.log("-------->", this.personal_asistencial.find(x => x.tipo_personal === tipo_personal));
      return this.personal_asistencial.find(function (x) {
        return x.tipo_personal === tipo_personal;
      });
    },
    handleDestroyPersonal: function handleDestroyPersonal(id, index) {//console.log(id,this.tipo_personal)
      ...
      return _asyncToGenerator( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.mark(function _callee2() {
        ...
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                _this.user_id_medico = user_id_medico; //console.log(this.personal_asistencial);

                if (!_this.personal_asistencial.some(function (x) {
                  return x.user_id === Number(_this.user_id_medico) && x.tipo_personal === _this.name;
                })) {
                  _context2.next = 5;
                  break;
                }
                ...
                alert("Ya has seleccionado a esta persona");
                _this.user_id_medico = "";
                return _context2.abrupt("return", false);

              case 5:
                _context2.prev = 5;
                formData = new FormData();
                formData.append("cat_quirofano_id", _this.n_quirofano);
                formData.append("user_id", _this.user_id_medico);
                formData.append("tipo_personal", _this.name);
                formData.append("solicitud_quirofano_id", _this.solicitud_quirofano_id);
                formData.append("_token", document.querySelector('meta[name="csrf-token"]').getAttribute('content'));
                _context2.next = 14;
                return Object(_helpers_customHelpers__WEBPACK_IMPORTED_MODULE_1__["post"])(location.origin + "/areaQuirofano/personal_asistencial/create", formData);
              ...
              case 14:
                result = _context2.sent;
                newData = _this.$store.state.listadoSolicitudesQx[_this.index_dia].dias[_this.index_solicitud].personal_asistencial.map(function (x) {
                  // console.log(x['user_id'],this.user_id_medico)
                  if (x['user_id'] === _this.user_id_medico) {
                    x.active = 0;
                  }
                ...
                _this.$store.commit('updateSolicitudQx2', [_this.index_dia, _this.index_solicitud, 'personal_asistencial', [].concat(_toConsumableArray(newData), _toConsumableArray(result))]); //console.log("solicitud->", this.$store.state.listadoSolicitudesQx[this.index_dia].dias[this.index_solicitud].personal_asistencial);
    ...
    childRef: function childRef() {// return this.$refs['input_' + this.name];
    },
    updateFormField: function updateFormField(event) {
      /*  this.$store.commit('updateFormField', {
           formName: "formReservacionQuirofano",
