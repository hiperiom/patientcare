      ...
      if (this.$store.state.formReservacionQuirofano['intervencion'].length === 0) {
        ...
        intervenciones.forEach(function (intervencion) {
          for (var _key in intervencion) {
            ...
            if (_key === 'description' && alerta.retorna) {
              if (intervencion[_key] === "") {
                alerta.title = "Escribe y confirma, al menos una Intervención, procedimiento o estudio a realizar.", alerta.retorna = false;
              }
            }

            if (_key === 'cirujano_principal' && alerta.retorna) {
              if (intervencion[_key].length === 0) {
                alerta.title = "Selecciona y confirma, al menos un cirujano principal", alerta.retorna = false;
              }
            }
          }
        });
