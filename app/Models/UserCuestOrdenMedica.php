...
class UserCuestOrdenMedica extends Model
{
    protected $table="user_cuest_orden_medica";
    static function index_episodio($episodio_id)
    {
        return UserCuestOrdenMedica::where('user_cuest_episodio_id',$episodio_id)
    ...
    static function store(Request $request)
    {
        $episode_id = UserCuestEpisodio::ultimoEpisodio_id($request->user_id);
                ...
                UserCuestOrdenMedica::where("user_id",$request->user_id)
                ->where("active",1)
                ->update(["active"=>0,"deleted_at"=>date('Y-m-d H:i:s', strtotime($request['created_at']))]);

                $model = new UserCuestOrdenMedica();
            ...
            return Response()->json(UserCuestOrdenMedica::show($request->user_id));
    ...
    {
        return UserCuestOrdenMedica::where("user_id",$user_id)->where("active",1)->first();
    }
    static function eliminar($id)
    {
        $model = UserCuestOrdenMedica::where('id',$id);
        $user_id = $model->value('user_id');
        $model->delete();
        return Response()->json(UserCuestOrdenMedica::show($id));
    }
}
